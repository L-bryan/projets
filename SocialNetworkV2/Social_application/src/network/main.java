...
public class main {
    public static void main(String[] args) throws IOException, FileNotFoundException, ClassNotFoundException, URISyntaxException {
        ...
        Information info4 = new ContactInformation("0482546298", "dylan@cookie.com");
        
        Information info10 = new NameInformation("Levy", "Sara");
        Information info20 = new SexInformation(Sex.WOMAN);
        Information info30 = new BirthDateInformation(26, 8, 1997);
        
        Information info100 = new NameInformation("Mendez", "Micaela");
        Information info200 = new SexInformation(Sex.WOMAN);
        Information info300 = new BirthDateInformation(9, 8, 1997);
        Information info400 = new HobbieInformation("Voyage");
        Information info500 = new HobbieInformation("Séries TV");
        
        net.createProfil(info1, info2, info3, info4);//0
        net.createProfil(info10, info20, info30);//1
        net.createProfil(info100, info200, info300,info400,info500);//2
        
        Node node0 = (Node) net.getMap().get(0);//dylan
        Node node1 = (Node) net.getMap().get(1);//sarah
        Node node2 = (Node) net.getMap().get(2);//micaela
        
        net.addLink(node0, node1, LinkType.FRIENDS);
        net.acceptLink(node1, node0,LinkType.FRIENDS);
        
        net.addLink(node1, node2, LinkType.FRIENDS);
        net.acceptLink(node2, node1,LinkType.FRIENDS);
        net.matchSuggestion();
        
        //net.addLink(node0, node2, LinkType.MATCH);
        /*for(Link l : node0.getPotentialMatchs()){
            if(l.getNodeSender()==node2 && l.getNodeReicever()==node0){
                l.setConfirmed(true);
            }
        }
        
        for(Link l : node2.getPotentialMatchs()){
            if(l.getNodeSender()==node0 && l.getNodeReicever()==node2){
                l.setConfirmed(true);
            }
        }*/
        //net.acceptLink(node0, node2, LinkType.MATCH_POTENTIAL);
        //net.acceptLink(node2, node0, LinkType.MATCH_POTENTIAL);
